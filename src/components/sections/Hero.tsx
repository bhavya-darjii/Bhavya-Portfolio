...
export function Hero() {
  ...
  // Track scroll progress through just the hero section
  const { scrollYProgress } = useScroll({
    target: sectionRef,
    offset: ["start start", "end start"],
  });

  // Smooth spring physics on scroll
  const spring = useSpring(scrollYProgress, { stiffness: 60, damping: 18 });

  // Text fades out + drifts up over first 40% of scroll
  const textOpacity = useTransform(spring, [0, 0.4], [1, 0]);
  const textY = useTransform(spring, [0, 0.4], [0, -24]);

  // Image: scale slightly, fade + blur — never fully disappears (min 0.14)
  const imageOpacity = useTransform(spring, [0.2, 0.88], [1, 0.14]);
  const imageScale = useTransform(spring, [0, 0.65], [1, 1.07]);
  const imageBlurRaw = useTransform(spring, [0.2, 0.88], [0, 14]);
  const imageFilter = useTransform(imageBlurRaw, (v) => `blur(${v}px)`);

  return (
    /*
     * Outer section is taller than the viewport (200svh on mobile, 200vh on desktop)
     * so the user has scroll room for the effects to play out.
     * The inner motion.div is `sticky top-0` so it stays pinned
     * while the user scrolls through the extra height.
     * This approach does NOT use `fixed` positioning, so it has
     * zero impact on the preloader, footer, or any other section.
     */
    <section
      id="home"
      ref={sectionRef}
    ...
    >
      {/* Sticky inner — pins to viewport top, plays effects as section scrolls past */}
      <motion.div
        initial={{ opacity: 0, scale: 0.98 }}
        animate={{ opacity: 1, scale: 1 }}
        transition={{ duration: 1, ease: [0.22, 1, 0.36, 1] }}
        className="hero-gradient-bg sticky top-0 overflow-hidden shadow-2xl shadow-teal-900/20 w-full"
        style={{ height: "100svh" }}
      >
        ...
        <div className="relative flex h-full flex-col">
          ...
          {/* ── PROFILE IMAGE — entry from right, scroll: scale + fade + blur ── */}
          <motion.div
          ...
          >
            {/* Mobile image — enters from right */}
            <motion.div
            ...
            {/* Desktop image — enters from right */}
            <motion.div
              initial={{ opacity: 0, x: 60 }}
              animate={{ opacity: 1, x: 0 }}
              transition={{ duration: 1.2, delay: 0.5, ease: [0.22, 1, 0.36, 1] }}
